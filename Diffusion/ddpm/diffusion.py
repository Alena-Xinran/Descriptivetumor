...
class Unet3D(nn.Module):
    ...
    def forward(
        ...
        input_channels = x.shape[1]  # 包括x、cond、text_embeddings的通道数
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, cond=None, text=None, noise=None, **kwargs):
        ...
        #print("Using text during training:", text)  # 打印文本输入
    ...
    def forward(self, x, cond=None, text=None, *args, **kwargs):
        ...
        if text is not None:
            ...
            text_embeddings = text_embeddings.expand(-1, -1, cond.shape[2], cond.shape[3], cond.shape[4])  # 匹配cond的空间维度
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        results_folder='/ccvl/net/ccvl15/xinran/outputs/',
        num_sample_rows=1,
        max_grad_norm=None,
        num_workers=20,
    ):
    ...
    def train(self, prob_focus_present=0., focus_present_mask=None, log_fn=lambda x: x):
        ...
        while self.step < self.train_num_steps:
            ...
            self.losses.append(loss.item())  # 保存当前的损失值到 self.losses 列表
            self.loss_line.set_xdata(range(len(self.losses)))  # 设置 x 轴数据
            self.loss_line.set_ydata(self.losses)  # 设置 y 轴数据
            self.ax.relim()  # 重新计算数据范围
            self.ax.autoscale_view()  # 自动缩放视图以适应新的数据
            plt.draw()  # 更新图像
            plt.pause(0.01)  # 小暂停以确保实时显示
