...
class CrossAttention(nn.Module):
    ...
    def forward(self, img_features, text_features):
        ...
        text_proj = text_proj.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # [batch, dim, 1, 1, 1]
        text_proj = text_proj.expand(-1, -1, depth, height, width)  # [batch, dim, depth, height, width]

        if channels != self.dim:
            raise ValueError(f"img_features channels ({channels}) must match dim ({self.dim}) in CrossAttention")
        
        fused_features = img_features + text_proj  # [batch, dim, depth, height, width]
        fused_features = fused_features.view(batch_size, self.dim, -1).transpose(1, 2)  # [batch, seq_length, dim]
        attended = self.attn(fused_features)  # [batch, seq_length, dim]
        attended = attended.transpose(1, 2).contiguous().view(batch_size, self.dim, depth, height, width)  # [batch, dim, depth, height, width]
        out = self.to_out(attended) + img_features  # [batch, dim, depth, height, width]
...
class Unet3D(nn.Module):
    ...
    def forward(
        ...
        # Combine mask with text embeddings
      # print(text)
        if text is not None:
...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x, t, cond=None, cond_scale=1., clip_denoised=True, text=None):
        ...
        if cond.shape[2:] != x.shape[2:]:
            cond = torch.nn.functional.interpolate(cond, size=x.shape[2:], mode='trilinear', align_corners=False)
    ...
    def p_sample_loop(self, shape, cond=None, cond_scale=1., text=None):
        ...
        if text is not None:
            inputs = self.denoise_fn.tokenizer(text, return_tensors="pt", padding=True, truncation=True).to(img.device)
            text_embeddings = self.denoise_fn.clip_model.get_text_features(**inputs)

            text_proj = nn.Linear(text_embeddings.shape[-1], cond.shape[1]).to(img.device)
            text_embeddings = text_proj(text_embeddings)
            text_embeddings = text_embeddings.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # Expand spatially
            text_embeddings = text_embeddings.expand(-1, -1, cond.shape[2], cond.shape[3], cond.shape[4])  # Match cond shape

            cond = torch.cat((cond, text_embeddings), dim=1)
        #print("cond1",cond.shape)
    ...
    def p_losses(self, x_start, t, cond=None, text=None, noise=None, **kwargs):
        ...
        #print("Using text during training:", text) 
    ...
    def forward(self, x, cond=None, text=None, *args, **kwargs):
        bs = int(x.shape[0] / 2)
        img = x[:bs, ...]
        mask = x[bs:, ...]
        mask_ = (1 - mask).detach()
        ...
        if text is not None:
            inputs = self.denoise_fn.tokenizer(text, return_tensors="pt", padding=True, truncation=True).to(img.device)
            text_embeddings = self.denoise_fn.clip_model.get_text_features(**inputs)  # (batch_size, embedding_dim)

            text_proj = nn.Linear(text_embeddings.shape[-1], cond.shape[1]).to(img.device)
            text_embeddings = text_proj(text_embeddings)
            text_embeddings = text_embeddings.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # (batch_size, channels, 1, 1, 1)
        ...
        return self.p_losses(img, t, cond=cond, text=text, *args, **kwargs)# trainer class
...
class Tester(object):
    ...
    def sample(self, batch_size, cond, text=None, cond_scale=1.):
        # Ensure that the text parameter is passed to the EMA model
        return self.ema_model.sample(batch_size=batch_size, cond=cond, cond_scale=cond_scale, text=text)
