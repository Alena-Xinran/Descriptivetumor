...
class UniformDataset(Dataset):
    ...
    def _transform(self, set_key, data_index):
        ...
        try:
            ...
            print(f"Error processing file: {data_i['name']}. Skipping. Error: {str(e)}")
            return None
...
class UniformCacheDataset(CacheDataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print(f"Error processing data at index {post_index}. Skipping. Error: {str(e)}")
            return self.__getitem__((index + 1) % self.__len__())
...
def get_loader(args):
    train_transforms = Compose(
        [
            ...
            #Lambda(lambda x: {**x, "text": (lambda choices: random.choice(choices))(choices=[val for val in x["text"] if val != previous_choice])}),
    ...
    def load_data(file, root_path, label_root_path):
        img_list = []
        lbl_list = []
        text_list = []
        name_list = []
        file_path= '/ccvl/net/ccvl15/xinran/ReportTumor/STEP2.DiffusionModel2/cross_eval/pancreas_tumor_data_early_fold/finetunr.txt'
