...
def main_worker(gpu, args):
    ...
    val_txt = os.path.join(datafold_dir, 'real_{}_val_{}.txt'.format(tumor_type, fold))

    for line in open(val_txt):
        tokens = line.strip().split()
        ct_path = tokens[0]  
        organ_tumor_label_path = tokens[1]  
        name = os.path.basename(organ_tumor_label_path).split('.')[0] 
    ...
    for i, batch_data in enumerate(train_loader):
        data_names = batch_data['name']
        print(f"Batch {i}: {data_names}")
