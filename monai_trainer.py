...
def train_epoch(model, loader, optimizer, scaler, epoch, loss_func, args, tumor_size_list):
    ...
    if args.organ_type == 'liver':
        ...
        sample_thresh = 0.5
    # model prepare
    vqgan, noearly_sampler= synt_model_prepare(device = torch.device("cuda", args.rank), fold=args.fold, organ=args.organ_model)
    ...
    for idx, batch_data in enumerate(loader):
        ...
        for bs in range(data.shape[0]):
            ...
            #if 'kidney_label' not in data_name or 'liver_label' in data_name or 'pancreas_label' in data_name:
            if random.random() > sample_thresh:
                ...
                if synthetic_tumor_type == 'early':
                    synt_data, synt_target = synthesize_early_tumor(healthy_data, healthy_target, args.organ_type, vqgan,  noearly_sampler)
